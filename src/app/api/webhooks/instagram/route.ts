...
async function processInstagramMessage(
  ...
  try {
    ...
    if (isManualMode) {
      const escalationActive = await hasActiveEscalation(companyId, `instagram-${senderId}`)
      if (escalationActive) {
        console.log('[Instagram] Chat has active escalation, skipping AI response')
        return
      }
      // Escalation resolved/missing/stale — auto-clear manual mode and let AI respond
      console.log('[Instagram] Manual mode was stale, auto-clearing and resuming AI')
      clearInstagramChatManualMode(companyId, senderId).catch(() => {})
    }
